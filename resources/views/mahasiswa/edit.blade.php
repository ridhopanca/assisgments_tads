...
-->
              ...
              <div class="kt_topbar_left">
                <h3 class="kt-subheader__title my-0">@isset($title)
                  ...
                  @if($errors->any())
									...
									@endif
									<form class="kt-form" method="post" action="{{ route('mahasiswa.update', $mahasiswa->id ) }}">
										@csrf
										<div class="kt-portlet__body">
                      <div class="form-group">
												<label>Nama</label>
												<input type="text" class="form-control" name="nama" aria-describedby="nama" placeholder="Masukkan Nama" value="{{ $mahasiswa->nama }}">
											</div>
                      <div class="form-group">
												<label>Alamat</label>
												<input type="text" class="form-control" name="alamat" aria-describedby="alamat" placeholder="Masukkan Alamat" value="{{ $mahasiswa->alamat }}">
											</div>
                      <div class="form-group">
												<label>No Telp</label>
												<input type="text" class="form-control" name="no_telp" aria-describedby="no_telp" placeholder="Masukkan No Telp" value="{{ $mahasiswa->no_telp }}">
											</div>
                      <div class="form-group">
                        ...
                        <select class="form-control" id="fakultas_id" name="fakultas_id">
                          ...
                          <option value="{{ $fk->id }}" {{ ( $fk->id == $mahasiswa->fakultas_id) ? 'selected' : '' }}>{{ $fk->nama_fakultas }}</option>
                          @endforeach
                        </select>
                      </div>
                      <div class="form-group">
    ...
    $( document ).ready(function() {
      var id = "{{ $mahasiswa->fakultas_id }}";
      loadProgramStudi(id);
    });
    function loadProgramStudi(id){
      var programid = "{{ $mahasiswa->program_studi_id }}";
      if(id == ''){
        var template = '';
      ...
      $.ajax({
        ...
        url: `/programstudi/${id}`,
          ...
          for( var i = 0; i < data.length;i++){
            template += `<option value="${data[i].id}" ${ (data[i].id == programid) ? 'selected' : '' } >${data[i].nama_program_studi}</option>`;
          }
          $('#program_studi_id').html(template);
      });
      }
